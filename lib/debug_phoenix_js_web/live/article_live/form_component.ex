defmodule DebugPhoenixJsWeb.ArticleLive.FormComponent do
  use DebugPhoenixJsWeb, :live_component
  ...
  def update(%{article: article} = assigns, socket) do
    changeset = Contents.change_article(article)
  ...
  def handle_event("validate", %{"article" => article_params}, socket) do
    changeset =
      ...
      |> Contents.change_article(article_params)
  ...
  defp save_article(socket, :edit, article_params) do
    case Contents.update_article(socket.assigns.article, article_params) do
      {:ok, article} ->
  ...
  defp save_article(socket, :new, article_params) do
    case Contents.create_article(article_params) do
      {:ok, article} ->
        notify_parent({:saved, article})
